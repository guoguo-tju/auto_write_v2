...
**根据用户选择执行：**
- 选择 A（调研）→ 调用 `research-expert` 进行素材搜集
- 选择 B（直接写作）→ 调用 `writing-executor` 开始写作

**配图说明：** 如果需要在文章中添加配图，可在写作完成后调用 `image-generator` Skill。写作时使用 `>>> 🖼️ IMAGE: 描述 <<<` 标记预留图片位置。
...
- v1.4.0 (2025-01-10): 添加配图说明，配图功能改为写作完成后主动调用。
